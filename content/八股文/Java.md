# Java基础
## OOP
* 面向对象编程：将对象作为程序的基本单元，对象中定义了属性以及操作这个属性的方法；
	* 三大特征：
		* 封装：将对象的属性隐藏在对象内部，不允许外部直接访问对象的内部属性，只对外暴露部分操作属性的方法；类似于我们不明白空调的内部构造，但可以通过空调遥控器来控制空调；
		* 继承：将一些对象的共有属性、功能抽象出来构造一个父类，子类通过继承该父类，快速获取这些共有的属性、功能，可以提高代码的复用性；
			* 子类拥有父类所有的属性和方法（包括私有属性、私有方法），但父类中的私有属性、私有方法子类无法访问，只是拥有；
			* 子类可以拥有自己的属性和方法，即子类可以对父类进行扩展；
			* 子类可以重写父类的方法；
		* 多态：一个对象具有多种状态，具体表现为`父类引用指向子类实例`；
			* 对象类型和引用类型之间具有继承/实现的关系；
			* 多态引用调用的方法具体是哪个类的方法要到程序运行区间才能确定；
			* 若子类重写了父类的方法，则多态引用调用的实际是子类的方法；
			* 多态不能调用只存在于子类中的方法；
* 面向过程编程：将函数作为程序的基本单元，通过对函数的划分，实现复用性和降低复杂度；

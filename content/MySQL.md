...
- `DATEDIFF(a.date, b.date)` ：计算日期a - 日期b的差值，即间隔多少天
- `TIMESTAMPDIFF(CONST_DATE, a.date, b.date)`：
	- CONST_DATE可以是DAY、HOUR、MINUTE等等时间单位常量，更加灵活
	- a.date是更小的日期，b.date是更大的日期
	- 函数作用是计算 b.date 到 a.date 间隔多少个时间常量
# 事务

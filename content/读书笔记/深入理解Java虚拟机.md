...
* 线程的实现：
	* 线程的实现主要有三种方式：
		* 用户线程：在用户空间实现的线程，不是由内核管理的线程，由用户的线程库来完成线程的管理；
	...
	* 用户线程与内核线程的对应关系：
		...
		* 多对多：
	* 用户线程：
		* 用户线程基于用户态线程库管理和实现，用户线程的TCB也是由用户线程库实现，操作系统无法感知用户线程TCB，只能看到整个进程的PCB；
		* 用户线程的创建、调度、销毁都完全在用户态实现，不需要操作系统内核的参与；
		* 用户线程模型：多个用户线程对应一个内核进程；
		* 优点：
		* 缺点：
	* 内核级线程：
		* 内核线程由操作系统管理，线程对应的TCB也是由操作管理；
		* 线程的创建、调度、销毁都由操作系统负责；
		* 内核线程模型：一个用户线程对应一个内核线程；
		* 优点：
		* 缺点：
	* 轻量级进程：
		* 轻量级进程Light-weight-process（LWP）是内核支持的用户线程，一个进程可有一个或者多个LWP，每个LWP跟内核线程是一对一映射的，LWP都是由一个内核线程所支持，LWP由内核管理并向普通进程一样被调度；
		* 一个进程代表程序的一个实例，而LWP代表程序的执行线程；
		* 在LWP之上也可以使用用户线程，LWP与用户线程的对应关系有三种：
			* 1:1，一个LWP对应一个用户线程；
			* 1:N，一个LWP对应N个用户线程；
			* M:N，M个LWP对应N个用户线程；

...
* Garbage First 垃圾回收器（G1）：
	...
	* `停顿时间模型`垃圾回收器：指定一个长度为M毫秒的时间片段内，消耗在垃圾回收器上的时间大概率不超过N毫秒；
	...
	* G1收集器运作过程：
		...
		* 并发标记：该阶段可与用户线程并发执行，对所有GC Roots引用链上的对象做扫描，找出要回收的对象，并且需要处理在快照记录下在并发时有引用变动的对象；
